...
function update_pending_user_email($old_email, $new_email) {
    global $con;
    
    // Check if new email is already taken
    $stmt = $con->prepare("SELECT user_id FROM users WHERE user_email = ? LIMIT 1");
    if (!$stmt) return ['success' => false, 'error' => 'Database error during collision check'];
    ...
    // Update email only for pending users
    $stmt = $con->prepare("UPDATE users SET user_email = ? WHERE user_email = ? AND user_status = 'pending_verification'");
    if (!$stmt) return ['success' => false, 'error' => 'Database error during update'];
    ...
    if ($ok && $affected > 0) {
        ...
        return ['success' => false, 'error' => 'Failed to update email or user not in pending state'];
    }
}
?>