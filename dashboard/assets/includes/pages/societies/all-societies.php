...
<script>
    ...
    function loadSocieties() {
        const ajaxUrl = window.siteUrl ? `${window.siteUrl}assets/includes/functions/ajax-handlers/ajax-get-societies.php` :
            '../assets/includes/functions/ajax-handlers/ajax-get-societies.php';
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    const rows = data.data.map((society, index) => {
                        ...
                        const logoUrl = society.society_logo ? (window.siteUrl + society.society_logo) :
                            'assets/images/default-society.png';
                        ...
                        if (society.society_status === 'pending_verification') {
                            actionsHtml = `<a
        href="<?= $dashboard_url ?>?type=societies&page=finish-verification&society_id=${society.society_unique_id}"
        class="btn btn-sm btn-primary-600 rounded-pill">Finish Verification</a>`;
                        } else {
                            actionsHtml = `
    ...
    `;
        ...
        <img src="${logoUrl}" alt="" class="flex-shrink-0 me-12 radius-8"
